...
- 애플리케이션 이름: `마케팅방주`
- 사용 API: **네이버 로그인**
- 제공 정보 선택: **이메일 주소 (필수) 하나만**
  - ⚠️ 이름·별명·프로필 사진 등은 **체크하지 마세요.** 네이버 검수는 선택한 정보마다
    "서비스 어디에서 쓰는지" 화면 캡처를 요구하고, 못 보여주면 반려합니다. 우리 사이트는
    이메일만 씁니다(마이페이지 표시, 산출물 전달).
...
3. **제공정보 활용처**: 이메일이 보이는 **마이페이지** 화면 캡처
4. **업종 선택**: 해당 없으면 "없음"
