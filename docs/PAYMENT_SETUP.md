...
## 멱등 처리 (적용됨)
- 웹훅은 `credit_payment(payment_id, ...)` RPC로 적립하며, `payment_events`
  테이블의 `payment_id` 유일 키로 **중복 적립을 차단**합니다(웹훅 재전송 안전).

## 남은 보강(권장)
- **결제 시도 기록**: 충전 시작 시 pending 행을 만들고 웹훅에서 상태 갱신(감사 추적).
- **금액 교차검증**: 결제 요청 금액과 PortOne 재조회 금액 일치 여부 확인(현재는 PortOne
  재조회 금액으로 적립하므로 위·변조 영향은 없음).
